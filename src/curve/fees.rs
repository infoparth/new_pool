...
pub struct Fees {
    ...
    /// Burn fees is the amount of tokens being burnt on every trade
    /// These tokens won't go to any wallet, instead be burnt
    /// Burn fee numerator
    pub burn_fee_numerator: u64,
    /// Burn fee denominator
    pub burn_fee_denominator: u64,
}
...
impl Fees {
    ...
    /// Calculate the burn fee for a trade
    pub fn burn_fee(&self, trading_tokens: u128) -> Option<u128> {
            calculate_fee(
                trading_tokens,
                u128::from(self.burn_fee_numerator),
                u128::from(self.burn_fee_denominator),
            )
        }
...
pub fn pre_trading_fee_amount(&self, post_fee_amount: u128) -> Option<u128> {
    if self.trade_fee_numerator == 0 || self.trade_fee_denominator == 0 {
        let pre_owner_fee = pre_fee_amount(
            post_fee_amount,
        ...
        pre_fee_amount(
            pre_owner_fee,
            self.burn_fee_numerator as u128,
            self.burn_fee_denominator as u128,
        )
    } else if self.owner_trade_fee_numerator == 0 || self.owner_trade_fee_denominator == 0 {
        let pre_trade_fee = pre_fee_amount(
            post_fee_amount,
        ...
        let pre_combined_fee = pre_fee_amount(
            post_fee_amount,
            (self.trade_fee_numerator as u128)
    ...
    pub fn validate(&self) -> Result<(), SwapError> {
        ...
        validate_fraction(self.burn_fee_numerator, self.burn_fee_denominator)?;
...
impl Pack for Fees {
    ...
    fn pack_into_slice(&self, output: &mut [u8]) {
        ...
        *burn_fee_numerator = self.burn_fee_numerator.to_le_bytes();
        *burn_fee_denominator = self.burn_fee_denominator.to_le_bytes();
    }
    ...
    fn unpack_from_slice(input: &[u8]) -> Result<Fees, ProgramError> {
        ...
        Ok(Self {
            ...
            burn_fee_numerator: u64::from_le_bytes(*burn_fee_numerator),
            burn_fee_denominator: u64::from_le_bytes(*burn_fee_denominator),  
        })
    }
}
...
mod tests {
    ...
    fn pack_fees() {
        ...
        packed.extend_from_slice(&burn_fee_numerator.to_le_bytes());
        packed.extend_from_slice(&burn_fee_denominator.to_le_bytes());
        let unpacked = Fees::unpack_from_slice(&packed).unwrap();
        assert_eq!(fees, unpacked);
    }
}
